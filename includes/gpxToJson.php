 ...
 * - Le denivele positif est calcule
...
function formatGPXExcursions($gpxFile)
{
    ...
    if (!$excursion) {
        return new WP_Error('erreur_gpx', 'Erreur lors de l\'ouverture du fichier gpx', array('status' => 500));
    }
    ...
    if (isset($excursion["trk"][1])) {
        return new WP_Error('erreur_gpx', 'GPX doit contenir un seul trkseg', array('status' => 500));
    } else if (!isset($excursion["trk"]["trkseg"]["trkpt"])) {
        return new WP_Error('erreur_gpx', 'GPX Mal formaté', array('status' => 500));
    } else if (count($excursion["trk"]["trkseg"]["trkpt"]) < 2) {
        return new WP_Error('erreur_gpx', 'Nombre de points insuffisants', array('status' => 500));
    }
    ...
    /* ----------------------- Calcul du denivele positif ----------------------- */
    $points[0]["pos"]=0;
    for ($i = 1; $i < sizeof($points); $i++) {
        $point = &$points[$i];
        $pointPrecedent = $points[$i - 1];
        $deltaAltitude = $point["alt"] - $pointPrecedent["alt"];

        if ($deltaAltitude > 0){
            // On ajoute la difference d altitude au denivele positif
            $point["pos"] = $pointPrecedent["pos"] + $deltaAltitude;
        }
        else{
            // On conserve le denivele positif precedent
            $point["pos"] = $pointPrecedent["pos"];
        }
    }
    ...
    foreach ($points as &$point) {
        $point["dist"] = round($point["dist"]);
        $point["pos"] = round($point["pos"]);
    }
